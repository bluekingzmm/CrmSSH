...
<BODY>
	...
	action="${pageContext.request.contextPath }/editLinkMan" method=post>
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<TABLE cellSpacing=0 cellPadding=5 border=0>
							<tr>
								<td>所属客户：</td>
								<td colspan="3"><select id="custSelect"
									name="linkman.customer.cust_id">
										<c:forEach items="${cList }" var="c">
											<option value="${c.cust_id }" ${c.cust_id==linkman.customer.cust_id? "selected":"" }>${c.cust_name}</option>
										</c:forEach>
								</select> </td>
						</tr>
							<TR>
								...
								<td><INPUT class=textbox id=sChannel2 style="WIDTH: 180px"
									maxLength=50 name="linkman.lkm_name"
									value="${linkman.lkm_name}"></td>
								<td>联系人性别：</td>
								<td><input type="radio" value="1" name="linkman.lkm_gender"
									<c:if test="${linkman.lkm_gender=='1' }">checked</c:if>>男

									<input type="radio" value="2" name="linkman.lkm_gender"
							...
							<TR>
								<td>联系人办公电话 ：</td>
								<td><INPUT class=textbox id=sChannel2 style="WIDTH: 180px"
									maxLength=50 name="linkman.lkm_phone"
									value="${linkman.lkm_phone}"></td>
								<td>联系人手机 ：</td>
								<td><INPUT class=textbox id=sChannel2 style="WIDTH: 180px"
									maxLength=50 name="linkman.lkm_mobile"
									value="${linkman.lkm_mobile}"></td>
						</TR>
							<tr>
